...
impl SizeInfo<f32> {
    ...
    pub fn new(
        ...
        let columns = columns.clamp(
            MIN_COLUMNS,
            pillar_config.width + pillar_config.slacka.unwrap_or(usize::MAX),
        );
...
impl Display {
    ...
    fn damage_from_point(&self, point: Point<usize>, len: u32) -> DamageRect {
        ...
        let line = point.line % size_info.physical_lines();
        let pillar = point.line / size_info.physical_lines();

        let x = size_info.padding_x()
            + pillar as u32 * size_info.cell_width()
            + point.column.0 as u32 * size_info.cell_width();
...
//=======
        let y = y_top - (line as u32 + 1) * size_info.cell_height();
        let width = len * size_info.cell_width();
        DamageRect { x: x as i32, y: y as i32, width: width as i32, height: size_info.cell_height() as i32 }
//>>>>>>> 9bc4328c (Add pillar support to display)
    }
